...
import { Zap, Plus, Search, Loader2, Pencil, Trash2, Inbox } from 'lucide-react';
import { getLeadSources, deleteLeadSource, getLeadSourcesSummary } from '../utils/leadSourceStorage';
...
export default function LeadSourcesDashboard() {
  ...
  const loadSources = async () => {
    try {
      ...
      setSources(data);
      setSummary(getLeadSourcesSummary(data));
    } catch (err) {
  ...
  const handleDelete = async (id, name) => {
    if (!window.confirm(`Delete lead source "${name}"? This cannot be undone.`)) {
      return;
    }
    ...
    try {
      ...
      alert('Error deleting lead source: ' + err.message);
    }
  };
  ...
  const filtered = sources.filter((source) => {
    ...
    if (!term) return true;
    return [source.name, source.description, source.createdBy, source.updatedBy]
      .filter(Boolean)
    ...
    <div className="min-h-full bg-surface px-5 py-8">
      <div className="mx-auto flex max-w-7xl flex-col gap-8">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <h2 className="text-2xl font-semibold text-slate-800">Lead Sources</h2>
            <p className="mt-1 text-sm text-slate-500">
              Manage lead source records stored in data/leadSources.json.
            </p>
          </div>
          <Link
            to="/lead-sources/new"
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
          <div className="flex items-center gap-4 rounded-lg bg-white p-5 shadow-sm">
            <Zap className="size-10 shrink-0 text-brand" />
            <div>
              <p className="text-xs font-semibold tracking-wide text-slate-500 uppercase">Total Sources</p>
          ...
          {filtered.length === 0 ? (
            <div className="px-5 py-16 text-center">
              ...
              <p className="mb-5 text-slate-500">
                {sources.length === 0
                  ? 'No lead sources yet. Create the first source.'
              ...
              <table className="w-full border-collapse text-sm">
                <thead>
                  <tr className="border-b-2 border-slate-100 bg-surface text-left uppercase text-[11px] tracking-[0.18em] text-slate-500">
                    <th className="px-4 py-4">Source Name</th>
                ...
                <tbody>
                  {filtered.map((source) => (
                    <tr key={source.id} className="border-b border-slate-100 transition-colors hover:bg-slate-50">
                      <td className="px-4 py-4 font-semibold text-slate-900">{source.name}</td>
                      <td className="px-4 py-4 text-slate-700">{source.description || '—'}</td>
                      <td className="px-4 py-4 text-slate-500">
                        <div>{source.createdDate ? new Date(source.createdDate).toLocaleDateString() : '—'}</div>
                        <div className="text-xs">{source.createdBy || 'System'}</div>
                      </td>
                      <td className="px-4 py-4 text-slate-500">
                        <div>{source.updatedDate ? new Date(source.updatedDate).toLocaleDateString() : '—'}</div>
                        <div className="text-xs">{source.updatedBy || 'System'}</div>
                      </td>
                      <td className="px-4 py-4">
                        <div className="flex justify-center gap-2">
                          <Link
                            to={`/lead-sources/${source.id}/edit`}
