...
import { createEmailGroup, updateEmailGroup, getEmailGroupById } from '../utils/emailGroupStorage';
...
export default function EmailGroupForm() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        setLoadError('');
        const group = await getEmailGroupById(id);
        if (!cancelled) {
          setFormData({
            name: group.name || '',
        ...
        if (!cancelled) {
          setLoadError('Could not load email group. It may have been deleted.');
        }
      } finally {
        if (!cancelled) {
          setLoading(false);
  ...
  const validateForm = () => {
    ...
    if (!formData.name.trim()) {
      newErrors.name = 'Email group name is required';
    }
    if (!formData.emailIds.trim()) {
      newErrors.emailIds = 'Email IDs are required';
    }
    return newErrors;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      emailIds: formData.emailIds.trim(),
    ...
    try {
      if (isEditing) {
        await updateEmailGroup(id, payload);
      } else {
        await createEmailGroup(payload);
      }
      setSubmitted(true);
      setTimeout(() => navigate('/email-groups'), 1500);
    } catch (err) {
      alert(err.message || 'Failed to save email group');
    } finally {
      setSaving(false);
    }
  };
      ...
      <div className="mx-auto max-w-3xl rounded-lg bg-white p-6 shadow-sm sm:p-10">
        <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h2 className="text-2xl font-semibold text-slate-800">
              {isEditing ? 'Edit Email Group' : 'Create New Email Group'}
            </h2>
            <p className="text-sm text-slate-500">
              {isEditing
                ? 'Update email group details.'
                : 'Add a new email group with semicolon-delimited addresses.'}
            </p>
          </div>
          <button
            type="button"
        ...
        {submitted && (
          <div className="mb-5 flex items-center justify-center gap-2 rounded border border-emerald-200 bg-emerald-50 px-4 py-4 text-sm font-medium text-emerald-800">
            ...
            {isEditing
              ? 'Email group updated successfully. Redirecting...'
              : 'Email group created successfully. Redirecting...'}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex flex-col gap-2">
            <label htmlFor="emailIds" className="text-sm font-semibold text-slate-800">
              Email IDs *
            </label>
            <textarea
              id="emailIds"
              ...
              onChange={handleChange}
              className={fieldClass(errors.emailIds)}
              rows="4"
              placeholder="john@example.com; jane@example.com"
            />
            {errors.emailIds && (
              <span className="text-xs font-medium text-red-500">{errors.emailIds}</span>
            )}
          </div>
